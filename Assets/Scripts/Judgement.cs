...
{
    readonly int miss = 200;
    readonly int good = 100;
    readonly int great = 50;
    ...
    public void Init()
    {
        ...
        notes.Add(note3);
        notes.Add(note4);
        noteObjects = NoteGenerator.Instance.toReleaseList;
    ...
    public void Judge(int line)
    {
        ...
        {
            // 노트 체력 감소
            NoteObject noteObj = noteObjects.Find(n => n.note.id == note.id);
            if (noteObj != null)
            {
                int prevHitPoint = noteObj.hitPoint;
                noteObj.hitPoint--;
                noteObj.UpdateHPText();
                Debug.Log($"Node {note.id} {prevHitPoint} to {noteObj.hitPoint}");
                // 완전히 처리되었을 경우에만 Dequeue
                if (noteObj.hitPoint <= 0)
                {
                    Debug.Log($"Complete node");
    ...
    IEnumerator IECheckMiss()
    {
        while (true)
        {
            ...
            for (int i = 0; i < notes.Count; i++)
            {
                ...
                NoteObject noteObj = noteObjects.Find(n => n.note == note);
